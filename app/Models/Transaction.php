    ...
    public static function sumCashInByPeriod(
        ...
        $sql = "
            SELECT 
                ...
                {$accountsTable}.id AS account_id,
                {$accountsTable}.name AS account_name,
                {$accountsTable}.type AS account_type,
                COALESCE(SUM(t.amount), 0) AS total_cash_in,
                COUNT(t.id) AS transaction_count
            FROM {$transactionTable} t
            INNER JOIN {$accountsTable} ON t.financial_account_id = {$accountsTable}.id
        ...
        if ($periodFormat === 'week' && $driver === 'mysql') {
            $sql .= " GROUP BY YEAR(t.created_at), WEEK(t.created_at, 1), {$accountsTable}.id, {$accountsTable}.name, {$accountsTable}.type";
        } else {
            $sql .= " GROUP BY {$periodExpr}, {$accountsTable}.id, {$accountsTable}.name, {$accountsTable}.type";
        }
